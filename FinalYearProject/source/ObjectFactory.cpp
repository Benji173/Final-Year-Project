...
b2Body* ObjectFactory::createBox(b2World* world, b2Vec2 position, b2Vec2 dimensions, float angle, b2BodyType type, float density) {
	b2Body* box;
	b2BodyDef boxDef;      
...
b2Body* ObjectFactory::createCircle(b2World* world, b2BodyType type, b2Vec2 position , float r)
{
	///Returns a circle with a specified type, parameters x, y and radius and default friction
	b2Body* circle;
	b2BodyDef circleDef;
	...
	circleShape->m_p.x = 0;
	circleShape->m_p.y = 0;
	circleShape->m_radius = r * PIXELSTOMETRES;

	//Create body
	circleDef.position.Set(position.x  * PIXELSTOMETRES, -position.y  * PIXELSTOMETRES);
	circleDef.type = type;
